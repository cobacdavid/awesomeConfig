...
local function couleurAlea()
    ...
    if secretFenetre.nb then
      ...
      return string.format("#%02X%02X%02X", nuance, nuance, nuance)
        ...
        local R = math.floor(math.random() * ecart) + secretFenetre.minNuance
        local G = math.floor(math.random() * ecart) + secretFenetre.minNuance
        local B = math.floor(math.random() * ecart) + secretFenetre.minNuance
                     ...
                     function()
                         if secretFenetre.challenge() then
                             ...
                             table.remove(secretFenetre.wiboxList,
                                          tableFind(secretFenetre.wiboxList, c))
                             --
...
tag.connect_signal("property::selected",
                   function(t)
                       --
                       for _, c in ipairs(secretFenetre.wiboxList) do
...
--
return setmetatable(secretFenetre, {__call=function(_, client, args)
